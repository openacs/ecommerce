...
if {[info exists product_id]} {
    set bottom_links "<li><a href=\"product?[export_url_vars product_id]\">Continue Shopping</a>\n"
} else {
...
group by p.product_name, p.one_line_description, p.product_id, u.offer_code, i.color_choice, i.size_choice, i.style_choice" {
    ...
    # Also print the price for a product of the selected options and the aforementioned delete option.
    append cart_contents "
    ...
    # Too bad I have to do another call to get the price. That is because ec_price_line returns canned html instead
    # of the price.
...
if { $product_counter != 0 } {
    ...
    "
    # List the states that get charged tax. Although not 100% accurate as shipping might be taxed too
    # this better than nothing.
    db_foreach tax_states "select tax_rate, initcap(state_name) as state from ec_sales_tax_by_state tax, us_states state where state.abbrev = tax.usps_abbrev" {
	append cart_contents "
          <tr>
