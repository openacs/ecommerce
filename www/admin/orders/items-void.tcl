# /www/[ec_url_concat [ec_url] /admin]/orders/items-void.tcl
ad_page_contract {

  @author Eve Andersson (eveander@arsdigita.com)
  @creation-date Summer 1999
  @cvs-id items-void.tcl,v 3.2.6.3 2000/08/17 15:19:15 seb Exp
...
if { [db_string num_non_void_items_select "select count(*) from ec_items where order_id=:order_id and product_id=:product_id and item_state<>'void'"] == 0 } {
    ad_return_complaint 1 "<li>These items are already void; perhaps you are using an old form.  <a href=\"one?[export_url_vars order_id]\">Return to the order.</a>"
    return
}

set n_items [db_string num_items_select "select count(*) from ec_items where order_id=:order_id and product_id=:product_id"]
...
doc_body_append "[ad_admin_header "Void $item_or_items"]

<h2>Void $item_or_items</h2>

[ad_admin_context_bar [list "../" "Ecommerce([ec_system_name])"] [list "index" "Orders"] [list "one?[export_url_vars order_id]" "One Order"] "$item_or_items"]
...
if { $n_items == 1 } {
    ...
    # this page
    set item_state [db_string item_state_select "select item_state from ec_items where order_id=:order_id and product_id=:product_id"]

    if { $item_state == "shipped" || $item_state == "arrived" || $item_state == "received_back" } {
	doc_body_append "<font color=red>Warning:</font> our records show that this item has already
    ...
    doc_body_append "Please confirm that you want to void this item.
    "

} else {
    # cases 3 & 4 (only differ by a warning message)
    set n_shipped_items [db_string num_shipped_items_select "select count(*) from ec_items where order_id=:order_id and product_id=:product_id and item_state in ('shipped','arrived','received_back')"]

    if { $n_shipped_items > 0 } {
	doc_body_append "<font>Warning:</font> our records show that at least one of these
    ...
    doc_body_append "Please check off the item(s) you wish to void.
    <table>
	...
	doc_body_append "<tr><td align=center>"
	if { $item_state == "void" } {
	    doc_body_append " (already void) "
	} else {
	    doc_body_append "<input type=checkbox name=item_id value=$item_id>"
	}
	doc_body_append "</td><td>$product_name; $price_name: [ec_pretty_price $price_charged]</td><td>$item_state</td></tr>"
    }
