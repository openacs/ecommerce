# /www/[ec_url_concat [ec_url] /admin]/orders/items-void-2.tcl
ad_page_contract {
  ...
  @cvs-id items-void-2.tcl,v 3.2.2.3 2000/08/17 15:19:15 seb Exp
...
db_transaction {
  ...
  set gift_certificate_amount [db_string gift_certificate_amount_select "select ec_order_gift_cert_amount(:order_id) from dual"]
  ...
  if { $n_items > 1 } {
    ...
    db_dml item_state_update "update ec_items set item_state='void', voided_date=sysdate, voided_by=:customer_service_rep where item_id in ([join $item_id_list ", "])"
  } else {
    db_dml item_state_update2 "update ec_items set item_state='void', voided_date=sysdate, voided_by=:customer_service_rep where order_id=:order_id and product_id=:product_id"
  }
  ...
  if { $certificate_amount_to_reinstate > 0 } {
    ...
    set certs_to_reinstate_list [db_list certs_to_reinstate_list_select "select u.gift_certificate_id
    ...
    set certificate_amount_used [db_string certificate_amount_used_select "select ec_order_gift_cert_amount(:order_id) from dual"]
	...
	if { $amount_to_reinstate > 0 } {
	  ...
	  set reinstatable_amount [db_string reinstatable_amount_select "select ec_one_gift_cert_on_one_order(:cert,:order_id) from dual"]

	  if { $reinstatable_amount > 0 } {
	    set iteration_reinstate_amount [ec_min $reinstatable_amount $amount_to_reinstate]
		    
	    db_dml reinstate_gift_certificate_insert "insert into ec_gift_certificate_usage
