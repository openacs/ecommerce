...
if { [expr $cash_amount_to_refund] > 0 } {
    ...
    if {![info exists creditcard_id] || ([info exists creditcard_id] && [empty_string_p $creditcard_id])} {
        ...
        append exception_text "<li>The refund requires using the creditcard that was used to purchase the items to be returned.</li>"
    }
    ...
    # The credit card has been re-entered. Check that the number is correct.
...
while { $refund_amount > 0 } {
	...
	limit 1"] } {
        ...
        if { $unrefunded_amount >= $refund_amount } {
            ...
            # Create refund financial transaction for the refund amount.
            set refund_transaction_id [db_nextval ec_transaction_id_sequence]
...
if { $certificate_amount_to_reinstate > 0 } {
    ...
    if { $certificate_amount_used < $certificate_amount_to_reinstate } {
        ...
        ad_return_error "Gift Certificate Error" "<p>We were unable to reinstate the customer's gift certificate balance because the amount 
...
if { [info exists page_title } {
    ...
    set title "Refund Complete"
}
set context [list [list index "Orders / Shipments / Refunds"] $title]

set order_url "one?[export_url_vars order_id]"
