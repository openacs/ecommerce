...
<a href="/admin[ec_url"]><%= [ec_url] %>admin/</a>

<li>data model:  examples in
<a href="/doc/sql/display-sql?url=/doc/sql/ecommerce.sql">/doc/sql/ecommerce.sql</a>

<li>procedures:  /tcl/ad-audit-trail
<li>Audit site: <a href="/<%= [ec_url] %>admin/audit-tables"><%= [ec_url] %>admin/audit-tables</a>
<li>Display table: <a href="<%= [ec_url] %>admin/audit-table"><%= [ec_url] %>admin/audit-table</a>
<li>Display record: <a href="<%= [ec_url] %>admin/audit"><%= [ec_url] %>admin/audit</a>
</ul>
...
Tcl procedures (<code>ec_audit_trail</code> and 
<code>ec_audit_trail_for_table</code>) that helps you display the old 
values of a row, including highlighting particular changed columns, (3)
a Tcl procedure (<code>ec_audit_delete_row</code>) that simplifies the
logging of a deleted row, and (4) an example user interface (
<code>audit-tables</code>, <code>audit-table</code>, <code>audit</code>) to retrieve and display audit histories.

<h3>Steps for Auditing a Table</h3>

We record old information in a separate audit table (see 
<a href="http://photo.net/sql/triggers">the triggers chapter of 
...
<li>Change any  script that deletes rows from an audited table.  It
should call <code>ec_audit_delete_row</code> with args key list, column 
...
<li>Insert a call to <code>ec_audit_trail</code> in an admin page to
show the changes made to a key. Insert a call to <code>ec_audit_trail_for_table</code> to show the changes made to an entire table over a specified period of time.

<li> <em>optionally</em> define two views to provide "user friendly"
audits.  Look at the 
<a href="/doc/sql/display-sql?url=/doc/sql/ticket.sql">ticket
...
Arguments for 
<a href="proc-one?proc_name=ec_audit_trail_for_table"><code>ec_audit_trail_for_table</code></a>
...
<li><b>main_table_name</b> Table that holds the main record. If sent an empty string as main_table_name, ec_audit_trail assumes that the audit_table_name has all current records.
...
Arguments for 
<a href="proc-one?proc_name=ec_audit_trail"><code>ec_audit_trail</code></a>
...
Arguments for <a href="proc-one?proc_name=ec_audit_delete_row"><code>ec_audit_delete_row</code></a>
