...
ad_proc ec_linked_thumbnail_if_it_exists { dirname {border_p "t"} {link_to_product_p "f"} } { 
 ...
 Otherwise it returns the empty string. } {
	...
	if { $link_to_product_p == "f" } {
		...
		set linked_thumbnail "<a href=\"[ec_url]product-file/$file_path/product.gif\"><img $border_part_of_img_tag width=[lindex $thumbnail_size 0] height=[lindex $thumbnail_size 1] src=\"[ec_url]product-file/$file_path/product-thumbnail.jpg\"></a>"

	    }
	} else {
	    set linked_thumbnail "<a href=\"product?[export_url_vars product_id]\"><img $border_part_of_img_tag width=[lindex $thumbnail_size 0] height=[lindex $thumbnail_size 1] src=\"[ec_url]product-file/$file_path/product-thumbnail.jpg\"></a>"
...
# a recreation of philg_hidden_input, since deprecated.

ad_proc ec_hidden_input {name value} {
...
ad_proc ec_product_file_directory { product_id } {
    Returns the directory that that the product files are located
under the ecommerce product data directory. This is the two lowest
order digits of the product_id.
} {
...
ad_proc ec_assert_directory {dir_path} "Checks that directory exists, if not creates it" {
	...
	# wtem@olywa.net -- 03-12-2001
	# we need a recursive mkdir
	# but we will hold off for now
	# create a list of directories
	# each element in the list must have the full path
	# then run file exists and ns_mkdir foreach path
	ns_mkdir $dir_path
	return 1
    }
}
